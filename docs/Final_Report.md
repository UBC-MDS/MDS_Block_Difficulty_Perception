...
-   To check the potential confounders, we stratify the `Faculty` and `Last Job` on variables `Age` and `Sex`. The plots above show that `Age` or `Sex` do NOT significantly affect on `Last Job` and `Faculty`.
